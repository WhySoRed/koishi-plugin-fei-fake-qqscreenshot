import { Context, Session,h, Schema, base64ToArrayBuffer } from "koishi";
...
export function apply(ctx: Context) {
  ctx.command("截图").action(async ({ session }) => {
    if (!session?.quote) return "去引用一哈";
    return await screenshotMaker(session);
  });
  const ruDian = new RuDian(ctx);
  if (ruDian?.RDOne) {
    ctx.command("截图入典").action(async ({ session }, message, trans) => {
      if (!session?.quote) return "去引用一哈";
      const base64String = (await screenshotMaker(session))
        .replace('<img src=\"', "")
        .replace('"/>', "");

      return ruDian.RDOne(base64String, message || session.quote.content, trans || "");
    });
  }

  async function screenshotMaker(session: Session) {
    const messageHtml = messageHtmlMaker(
      session.quote.user.avatar,
      session.quote.user.name,
      session.quote.content
    );
    const html = screenshotHtmlMaker(
      timeStamp2time(session.quote.timestamp).slice(0, -3),
      messageHtml,
  ...
  ctx.command("截图测试").action(({ session }) => {
  ...
  function messageHtmlMaker(avatarUrl: string, name: string, message: string) {
    return `
...
`;
  ...
  function singleImgMessageHtmlMaker() {}

  function screenshotHtmlMaker(
    timeString: string,
    messageHtml: string,
    guildName: string,
    guildSize?: number
...
<div id="canvas" style="width: 1240; background-color: #EFEFEF;">
  ...
  <div class="fake-time">${timeString}</div>
  <div class="fake-guildname">${guildName}${
      guildSize ? "(" + guildSize + ")" : ""
    }</div>
  ${messageHtml}
